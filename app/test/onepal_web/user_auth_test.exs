defmodule OnepalWeb.UserAuthTest do
  use OnepalWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Onepal.Accounts
  alias Onepal.Accounts.Scope
  alias OnepalWeb.UserAuth

  import Onepal.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, OnepalWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      OnepalWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        OnepalWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
